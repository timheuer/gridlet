...
struct ModelTests {
  ...
  @Test("PersistenceService stores solved words uniquely in uppercase")
  func solvedWordPersistence() throws {
    ...
    try persistence.appendSolvedWords(["stare", "TALK", "stare", "Walk"])

    #expect(persistence.loadSolvedWords() == ["STARE", "TALK", "WALK"])
  }

  @Test("Puzzle completion records solved words for future exclusion")
  func puzzleCompletionRecordsSolvedWords() throws {
    let tempDirectory = FileManager.default.temporaryDirectory
      .appendingPathComponent(UUID().uuidString, isDirectory: true)
    try FileManager.default.createDirectory(at: tempDirectory, withIntermediateDirectories: true)
    defer { try? FileManager.default.removeItem(at: tempDirectory) }
    ...
    let puzzle = PuzzleDefinition(
      ...
      gridSize: .five,
      cells: cells,
      words: [WordEntry(direction: .across, text: "CAT", clue: "Pet", startRow: 0, startCol: 0)]
    )
    let gameState = GameState(puzzleId: puzzle.id, isDaily: false, gridSize: .five)
    let viewModel = PuzzleViewModel(puzzle: puzzle, gameState: gameState, persistence: persistence)

    viewModel.selectCell(row: 0, col: 0)
    viewModel.enterLetter("C")
    viewModel.enterLetter("A")
    viewModel.enterLetter("T")

    #expect(viewModel.isCompleted)
    #expect(persistence.loadSolvedWords() == ["CAT"])
  }
  ...
  func wordEntryCellsAcross() {
    let word = WordEntry(
      direction: .across, text: "HELLO", clue: "Greeting", startRow: 1, startCol: 0)
  ...
  func puzzleDefinitionAIDiagnostics() throws {
    ...
    #expect(
      decoded.aiGenerationDetail
        == "Accepted 12 validated AI entries; required at least 40 to use AI output.")
  }
